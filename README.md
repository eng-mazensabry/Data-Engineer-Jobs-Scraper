...
A Selenium + Python script that scrapes "Data Engineer" job listings from Wuzzuf (first 3 result pages) and exports them to a CSV file.

Author: Mazen Sabry

What it does
Opens Wuzzuf's Egypt jobs page in Chrome and searches for Data Engineer.
Goes through the first three pages of results, clicking the Next button between pages.
For every listing, extracts the title, company, location, required experience, and the job link.
Visits each job's own page and extracts the full job description.
Removes duplicate listings and saves everything to wuzzuf_jobs.csv (UTF-8 with BOM, so Arabic text displays correctly in Excel).
Extracted fields
Column	Description
